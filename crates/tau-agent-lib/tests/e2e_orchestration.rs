...
fn session_tree_budget_enforcement() {
    ...
    let db = tau_agent_lib::db::Db::open(&dir.path().join("test.db")).unwrap();

    // Create root with budget 3
    let root = tau_agent_lib::db::StoredSession {
        id: "root".into(),
        model: tau_agent_lib::providers::mock::mock_model(),
        system_prompt: None,
        cwd: Some("/tmp".into()),
    ...
    let c1 = tau_agent_lib::db::StoredSession {
        id: "c1".into(),
        model: tau_agent_lib::providers::mock::mock_model(),
        system_prompt: None,
        cwd: Some("/tmp".into()),
    ...
    let c2 = tau_agent_lib::db::StoredSession {
        id: "c2".into(),
        model: tau_agent_lib::providers::mock::mock_model(),
        system_prompt: None,
        cwd: Some("/tmp".into()),
    ...
    let gc1 = tau_agent_lib::db::StoredSession {
        id: "gc1".into(),
        model: tau_agent_lib::providers::mock::mock_model(),
        system_prompt: None,
        cwd: Some("/tmp".into()),
...
fn session_tree_recursive_delete() {
    ...
    db.append_message(
        "gc1",
        &tau_agent_lib::types::Message::User(tau_agent_lib::types::UserMessage::text("hello")),
    )
    .unwrap();
...
fn session_model_inheritance() {
    ...
    let mut parent_model = tau_agent_lib::providers::mock::mock_model();
    ...
    let resolved_model = child_model
        .as_ref()
        .map(|_| tau_agent_lib::providers::mock::mock_model())
...
fn orchestration_tool_definitions() {
    ...
    assert!(names.contains(&"session_restore"));
    ...
    assert!(names.contains(&"skill_patch"));
    assert!(names.contains(&"session_search"));
    assert!(names.contains(&"schedule"));
    assert!(names.contains(&"task_decompose"));
...
fn protocol_wait_sessions_roundtrip() {
    ...
    let resp = Response::SessionsCompleted {
        results: vec![tau_agent_lib::protocol::SessionResult {
            session_id: "s1".into(),
            status: "done".into(),
...
fn protocol_session_info_tree_fields() {
    let info = tau_agent_lib::protocol::SessionInfo {
        id: "s1".into(),
    ...
    let parsed: tau_agent_lib::protocol::SessionInfo = serde_json::from_str(&json).unwrap();
...
fn spawn_child_chat_produces_messages() {
    ...
    match resp {
        Response::Messages { messages } => {
            ...
            assert!(
                matches!(&messages[1], tau_agent_lib::types::Message::Assistant(a) if a.text().contains("Child response"))
            );
        }
...
fn second_child_completion_notifies_parent() {
    ...
    let extract_user_text = |msg: &tau_agent_lib::types::Message| -> String {
        match msg {
            tau_agent_lib::types::Message::User(u) => u
                .content
                .iter()
