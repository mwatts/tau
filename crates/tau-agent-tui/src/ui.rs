...
fn draw_input(frame: &mut Frame, app: &App, theme: &Theme, area: Rect) {
    ...
    // If there are pending attachments, reserve the top inner row for a
    // chip line listing the queued files. Falls back to the original
    // single-region textarea when nothing is pending so the layout is
    // unchanged for the common case.
    if app.pending_attachments.is_empty() || inner.height < 2 {
    ...
    let mut spans: Vec<Span<'static>> = Vec::new();
    for (i, att) in app.pending_attachments.iter().enumerate() {
        if i > 0 {
            spans.push(Span::raw("  "));
        }
        spans.push(Span::styled(
            format!(
                "\u{1F4CE} {} ({})",
                att.display_name,
                crate::app::format_byte_size(att.decoded_bytes)
            ),
            dim,
        ));
    }
    spans.push(Span::styled(
        "  \u{2022} /clear-attach to remove".to_string(),
        dim,
    ));
...
fn draw_session_picker(frame: &mut Frame, app: &App, theme: &Theme, area: Rect) {
    ...
    if app.picker_sessions.is_empty() {
        ...
        for &(session_idx, depth, is_last) in &tree {
            ...
            // State indicator: ● (idle/green), ▶ (active/live), * (current)
            let (state_char, state_color) = if is_current {
                ("*", theme.accent)
            } else if session.is_live {
                // Actively running a chat turn right now.
                ("▶", theme.warning)
            } else {
